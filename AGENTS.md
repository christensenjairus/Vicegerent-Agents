...
- Preserve the client-side protected-branch guard across `development`, `main`, `master`, and `production`. Changes under `images/hermes/git-guard/` must keep its tests green and the branch list aligned with the GitHub and GitLab Cerbos policies. Treat forge-side branch protection as the security boundary.
