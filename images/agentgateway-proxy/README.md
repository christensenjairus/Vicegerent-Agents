...
The image `TAG` is `<AGW_VERSION>-rev<N>` (e.g. `v1.3.1-rev1`), the same scheme hermes-agent uses: bump `-rev<N>` on every rebuild that changes what the image contains — `patches/`, the `Dockerfile`, the Rust builder base — and reset to `-rev1` when `AGW_VERSION` bumps. The cluster pulls `IfNotPresent`, so a same-tag rebuild is never redeployed; without the suffix a new patch would silently ship as the old image. Renovate reads the `-rev` suffix as ordered via the `versioning: loose` rule keyed on `harbor.hahomelabs.com/vicegerent/agentgateway` in `renovate.json`. The patched-vs-stock distinction is carried by the **registry path** (`harbor.hahomelabs.com/vicegerent/agentgateway` vs upstream `ghcr.io/agentgateway/agentgateway`).
