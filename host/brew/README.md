...
Every `vicegerent start` and `vicegerent mcp start` runs `check` and automatically runs `apply --yes` before MCP startup when anything is drifted. Independent formula and notifier probes run concurrently so the clean path is bounded by the slowest probe rather than their cumulative runtime; all Homebrew mutations remain serialized.
