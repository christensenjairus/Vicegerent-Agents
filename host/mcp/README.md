...
Incoming webhook ingress is not part of this host stack. The cluster's dedicated `webhook-listener` Deployment owns the shared Cloudflare Tunnel and provider signing Secrets; see [`images/webhook-listener/README.md`](../../images/webhook-listener/README.md).
...
`thv` containerizes the k8s server, so it lives in the Docker VM and cannot reach a host-loopback API. With a blank `kubeconfig` param, it targets the Kind cluster selected by `VICEGERENT_KUBE_CONTEXT` (default: `kind-vicegerent`): the workload discovers that control plane's Docker network, uses `--isolate-network=false`, and mounts kind's `--internal` kubeconfig (`kind get kubeconfig --name <cluster> --internal`, server `https://<cluster>-control-plane:6443`) read-only via `KUBECONFIG` + `--kubeconfig`.
...
**Workload health.** It polls `thv list`'s own status for every currently-*enabled* workload and re-posts a macOS notification every poll while one is not `running`, so dismissing it never hides an unfixed failure for more than the poll interval. While it is running again, every poll removes that workload's notification group, so a failed or stale removal is retried until Notification Center converges. This exists because an OAuth-backed remote (`notion`, `linear`) losing its token doesn't just make individual tool calls fail - the whole workload drops out of `thv list`/vMCP's aggregation entirely (`unauthenticated`/`auth_retrying`/`error` in `thv`'s own state, or missing altogether) and stays down until something explicitly restarts it. A static-credential backend (`jira`'s API token, and `kubernetes`/`aws`/`aws_profiles` via `apply:aws_config`) can hit the same "workload just isn't running" state for other reasons too (a crashed container, a bad image pull), so this check is generic across every enabled backend rather than hand-picking OAuth ones.

**AWS credentials.** Whenever the `aws` server is enabled (nothing else here depends on AWS credentials), the same loop also watches that backend's credentials and warns *before* they expire, not only after. It re-posts an active warning every poll until the credentials are healthy again. Two signals:
...
Notifications go through the repository-owned native helper in [`host/notifier`](../notifier), not `osascript` or `terminal-notifier`. Host-package reconciliation compiles the helper, builds and signs its application bundle under `~/.vicegerent/notifier`, registers it with LaunchServices, and requests notification authorization. Its CLI writes failures to stderr and uses Apple's supported `UserNotifications` framework, so each watcher group is a native request identifier that can be queried and removed exactly.

Four more gotchas found live while actually watching this fire against a real expired session (handled in the shared notification helpers):

- **Persistent alerts require the Persistent Alert Style.** The default Temporary style leaves the screen after a few seconds. Host-package reconciliation requires **System Settings → Notifications → Vicegerent → Alert Style → Persistent** so an unresolved health failure remains onscreen.
- **Persistent alerts must use a stable identifier.** Every watcher alert uses its group as the native `UNNotificationRequest` identifier, so an unhealthy poll leaves a delivered alert alone or recreates one the user dismissed rather than stacking duplicates.
- **A long-lived supervisord can lose its connection to the current GUI login session.** Every process a daemon forks inherits whatever GUI-session bootstrap namespace that daemon had at its own creation time, no matter how freshly the forked child itself was spawned. `_notify`/`_notify_clear` dispatch the signed helper via `launchctl asuser $(id -u) ...` so it executes inside the current session instead of the calling process's possibly stale namespace.
- **A notification-service call must not block the watcher.** Every helper invocation has a short timeout. Unhealthy posts and healthy removals are retried on every poll, so an unavailable macOS notification service cannot suppress later backend or credential checks.
