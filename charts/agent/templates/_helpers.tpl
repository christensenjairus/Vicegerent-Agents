...
Before creating a worktree from a persistent `/workspace/<repo>` clone, run `git worktree prune`, then fetch the relevant remotes with `git fetch --prune --no-tags`. Prefer the current `upstream/HEAD`, then `upstream/main` or `upstream/master`, before falling back to `origin/HEAD`, `origin/main`, or `origin/master`; a personal fork's `origin` may be far behind its canonical `upstream`. Create the new worktree from that freshly fetched remote ref. Never pull, reset, rebase, clean, delete branches, or automatically update an existing task worktree as part of hygiene; report dirty, unmerged, or ambiguous stale worktrees instead of removing them.
{{- end -}}
...
Mnemosyne is the only memory store; use it for facts, preferences, and insights. Its memories are shared by Hermes, Claude Code, Codex, and OpenCode, so never maintain a harness-specific memory store. When grounded tool output, a user correction, or an investigation yields a durable, non-obvious fact that would prevent a future mistake, proactively save a compact declarative Mnemosyne memory in the same turn without asking for confirmation. Do not retain transient task state, raw logs, speculative conclusions, PR or issue identifiers, or duplicated documentation. The same is true of skills: the platform owns the canonical tree at `/opt/data/skills`, published to every harness through its native skill-discovery path; skills created or updated from any harness must remain useful across all four. Skills are shared procedural memory: when work yields a reusable workflow, corrects an existing procedure, or uncovers a non-obvious pitfall, create or update a skill rather than leaving the learning only in chat. Read a skill before modifying it, prefer the canonical tree over harness-local copies, and keep skills portable across all four. Do not create skills for one-off task progress.
{{- if .Values.obsidian.vaultPath }} `OBSIDIAN_VAULT_PATH` points to the shared, self-saving Obsidian vault at `{{ .Values.obsidian.vaultPath }}`. The vault is an Open Knowledge Format (OKF) bundle; before reading or writing it, consult its root `index.md` to locate and read the vault's complete OKF specification, then follow that local spec for concept frontmatter, bundle-relative Markdown links, indexes, and changelogs. Use the `obsidian` skill for vault reads, writes, and search, but the vault's own OKF spec overrides generic Obsidian conventions. Never use Git in the vault: do not initialize, inspect, stage, commit, push, pull, rebase, or create a worktree there. Treat the vault as a self-saving folder; any scheduled Git backup is operator-managed outside the agent's vault workflow. Keep the vault, skills, and Mnemosyne as separate systems: durable human-reviewable knowledge belongs in the vault, reusable procedures belong in skills, and Mnemosyne should hold short recall facts or pointers to vault paths rather than copied vault content.{{- end }}
{{- end -}}
...
For broad discovery across the persistent Git clones under `/workspace`, use the local daemonless Zoekt index: run `repo-map index` inside one checkout or pass explicit repository paths, then use `repo-map search -r <query>` and `repo-map list`; `-r` prefixes results with their repository so same-named paths are unambiguous; reserve `repo-map index --all` for deliberate whole-workspace indexing because indexes consume persistent disk. Prefer `repo-map` over shell search or opening files whenever the goal is locating something across a repository or across repositories, rather than confirming exact content in a file already identified; scope queries tightly rather than searching everything unfiltered, and consult `repo-map --help` for its query syntax. Zoekt is a navigation aid, not current-file evidence: it indexes committed Git content, so read the actual file and revision before making claims or editing, and use ordinary file search for uncommitted worktree changes.
{{- end -}}
...
{{ include "vicegerent-agent.workspaceGitHygiene" . | trim }}
...
terminal:
  ...
  # Keep tool subprocesses on the generic OS HOME instead of rewriting them under the Hermes-specific home.
  home_mode: real
web:
